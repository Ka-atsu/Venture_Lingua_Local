...
function PracticeIlocanoMainContent() {
    ...
    useEffect(() => {
        const savedState = JSON.parse(localStorage.getItem('completedChallengesIlocano'));
        ...
        if (location.state && location.state.isCompleted !== undefined) {
            ...
            if (category && level) {
                setCompletedChallenges(prevState => {
                    ...
                    localStorage.setItem('completedChallengesIlocano', JSON.stringify(updatedState)); // Save to localStorage
                    return updatedState;
                });
            }
        }
    ...
    const gotoChallenge = (category, level) => {
        ...
        navigate(`/ilocanoPractice/${category}/${level}`, { 
...
export default PracticeIlocanoMainContent;