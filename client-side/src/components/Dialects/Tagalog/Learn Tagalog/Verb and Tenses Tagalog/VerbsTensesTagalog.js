...
function VerbsTensesTagalog() {
  ...
  // Use the same naming convention as your phrases component
  ...
  return (
    <Container fluid className="bg-dark p-5 vh-100">
      ...
      <h2 className="text-center mb-3 text-white"> {groups[currentSet].group}</h2>
      ...
      <Row className="d-flex justify-content-center align-items-center" style={{ height: '65vh' }}>
        {groups[currentSet].tenses.map((item, index) => (
            ...
            >
              <div className="tagalog-flashcard-inner">
                <div className="tagalog-flashcard-front flex-column">
                  ...
                  <p>
                    <strong>Base Form:</strong> {groups[currentSet].group.split('(')[1].replace(')', '')}
                  </p>
                </div>
      ...
      <Row className="d-flex w-100 justify-content-between mt-5">
        ...
        <Col xs="auto">
          <Button 
            ...
            onClick={nextSet} 
            disabled={currentSet + 1 >= groups.length} 
            className="btn-lg rounded-pill mx-2"
          >
            Next Word
          </Button>
